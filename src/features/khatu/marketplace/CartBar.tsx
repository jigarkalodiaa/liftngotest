...
export default function CartBar({ onOpenDrawer, onCheckout }: CartBarProps) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-30 border-t border-stone-200 bg-white/98 px-3 py-3 shadow-[0_-12px_40px_rgba(28,25,23,0.08)] backdrop-blur-md">
      <button
        ...
        className="mx-auto flex w-full max-w-xl items-center gap-3 rounded-2xl border border-stone-200 bg-[var(--khatu-cream)] px-3 py-2.5 text-left sm:max-w-2xl"
      >
      ...
      <button
        type="button"
        onClick={onCheckout}
        className="mx-auto mt-2 flex h-12 w-full max-w-xl items-center justify-center rounded-xl bg-[var(--color-primary)] text-sm font-semibold text-white sm:max-w-2xl"
      >
        Proceed to checkout
      </button>
    </div>
