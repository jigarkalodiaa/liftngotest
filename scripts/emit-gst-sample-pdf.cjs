/**
 * One-off emitter: writes a minimal valid PDF to public/samples/.
 * Run: node scripts/emit-gst-sample-pdf.cjs
 */
const fs = require('fs');
const path = require('path');

function buildGstFlagshipSamplePdfBuffer() {
  const blocks = [];
  let total = 0;
  const offsets = [];
...
const out = path.join(__dirname, '../public/samples/liftngo-flagship-monthly-bulk-gst-report-sample.pdf');
fs.mkdirSync(path.dirname(out), { recursive: true });
const buf = buildGstFlagshipSamplePdfBuffer();
